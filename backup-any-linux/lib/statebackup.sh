...
validate_state_file_safe() {
    ...
    # Level 6: File-type-specific required fields
    case "$file_type" in
        backup)
            if ! jq -e '.filesystem_map' "$state_file" >/dev/null 2>&1; then
                log WARN "Validation failed: backup state missing filesystem_map"
                return 1
            fi
            ;;
        yesterday)
            if ! jq -e '.deleted_files' "$state_file" >/dev/null 2>&1; then
                log WARN "Validation failed: yesterday state missing deleted_files"
                return 1
            fi
            ;;
        permanent)
            if ! jq -e '.permanently_deleted_files' "$state_file" >/dev/null 2>&1; then
                log WARN "Validation failed: permanent state missing permanently_deleted_files"
                return 1
            fi
            ;;
        directory)
            if ! jq -e '.alignment_history' "$state_file" >/dev/null 2>&1; then
                log WARN "Validation failed: directory state missing alignment_history"
                return 1
            fi
            ;;
    esac
...
get_state_age_seconds() {
    ...
    # Sanity check: timestamp should be after 2020 (1577836800) and before 2100
    if [[ $ts_epoch -lt 1577836800 ]] || [[ $ts_epoch -gt 4102444800 ]]; then
        log WARN "Timestamp out of reasonable range: $ts_epoch"
        echo "unknown"
        return 1
    fi
...
recover_single_state_file() {
    ...
    # Ensure cleanup on exit (even if function interrupted)
    trap "rm -f '$temp_s3_file' 2>/dev/null" RETURN
    
    local s3_valid=false
    local s3_age="unknown"
    if s3_download "$s3_path" "$temp_s3_file" 2>/dev/null; then
...
decide_state_version() {
    ...
    # Validate ages are numeric
    if ! [[ "$local_age" =~ ^[0-9]+$ ]] || ! [[ "$s3_age" =~ ^[0-9]+$ ]]; then
        log WARN "Age values not numeric (local: $local_age, s3: $s3_age) - defaulting to local"
        echo "keep_local"
        return 0
    fi
    ...
    if [[ $age_diff -lt 0 ]]; then
        local age_diff_abs=$((-age_diff))  # Proper absolute value
